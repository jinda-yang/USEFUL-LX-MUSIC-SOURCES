...
class RateLimiter {
  ...
  checkLimit() {
    const now = Date.now();
    const clientId = 'global'; // 使用全局限制，可根据需要改为按IP或用户限制
    
    // 定期清理过期记录
    if (now - this.lastCleanup > RATE_LIMIT_CONFIG.cleanupInterval) {
      this.cleanupExpired();
    ...
    // 计算重置时间（最早请求的时间 + 时间窗口）
    const resetTime = clientRequests.length > 0 ? 
      Math.min(...clientRequests) + RATE_LIMIT_CONFIG.timeWindow : now;

    return {
      allowed,
      remaining: Math.max(0, RATE_LIMIT_CONFIG.maxRequests - requestCount - (allowed ? 1 : 0)),
      resetTime,
      resetIn: Math.max(0, Math.ceil((resetTime - now) / 1000 / 60)) // 剩余分钟数
    };
  }
